...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        int max_occur_idx = -1, max_occur = 0;
        for (int j = 0; j < last_idx; j++)
        {
            int occur = 0;
            for (int k = 0; k < last_idx; k++)
            {
                if (from_arr[k].sin_addr.s_addr == from_arr[j].sin_addr.s_addr)
                {
                    occur++;
                }
            }
        ...
        if (max_occur_idx != -1)
        {
            if (is_dest && from_arr[max_occur_idx].sin_addr.s_addr == dest_addr.sin_addr.s_addr)
            {
                printf("Destination reached at hop %d with ip %s\n", ttl, inet_ntoa(from_arr[max_occur_idx].sin_addr));
                from_addr = from_arr[max_occur_idx];
                exit(0);
            }
            else
            {
